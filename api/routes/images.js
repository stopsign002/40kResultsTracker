...
router.post('/:gameId/images', requireAuth, express.json({ limit: '12mb' }), async (req, res, next) => {
  try {
    ...
    // Tagging the terrain shot at upload time, the way the live tracker's Setup
    // step does. Inserted unflagged and set afterwards on purpose: the partial
    // unique index would reject the INSERT while the previous shot still holds
    // the flag, and re-shooting the table should demote that one, not fail.
    if (req.body?.isMap === true) {
      await pool.query('UPDATE game_images SET is_map = FALSE WHERE game_id = $1 AND is_map', [gameId]);
      row = (await pool.query(
        'UPDATE game_images SET is_map = TRUE WHERE id = $1 RETURNING *', [row.id])).rows[0];
    }
