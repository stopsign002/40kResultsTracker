...
| `game-write.js` | `createGame(client, body, actorUserId)`, `resolvePlayerIdentities`, `resolveGameLookups`, `insertPlayerChildren`, `recordBannerFirstSeen`, `detachmentList`, `joinDetachments`, `promoteDetachments`, `notifyGameLogged`, `FORCE_DISPOSITIONS` | The single INSERT path into `games` + `game_players` + children, shared by `POST /games` and `POST /drafts/:id/submit`. Call `createGame` inside `withTx`. The caller keeps the surrounding pipeline (validate → `resolvePlayerIdentities` → `computeFinalScores` → `resolvePlayerTimes` before; `audit` → `broadcast` → `notifyGameLogged` after). `PUT /games/:id` keeps its own delete-then-reinsert body but shares these helpers. Two reference-data rules live in here — see "Card ids on the write path" below. `promoteDetachments` is the third: every detachment a saved game carries joins that faction's `detachments` library (case-insensitively), so the suggestion list is a real table rather than something inferred from game history. That is the OPPOSITE of the match-only rule for secondary cards, and deliberately so — a bad detachment is fixable from Admin → Detachments, which is exactly what the card case lacked. |
...
| `mail.js` | `notify(subject, text)`, `isFixtureActor(username)` | Fire-and-forget POST to the shared `mailer` container. **No-ops unless `MAILER_URL` + `MAILER_TOKEN` are set**, never throws, never blocks — a mail hiccup must not fail a game save. 10s `AbortSignal.timeout`. Two callers: `notifyGameLogged` in `game-write.js` (a game was filed) and `notifyGameStarted` in `routes/drafts.js` (a live game left setup). **Both must gate on `isFixtureActor()` first** — they fire on rows the integration suite creates against the live API, and a run used to mail the operator a dozen times. It matches the suite's own `zz_test_` username prefix, so there is no toggle to leave in the wrong state; unsetting `MAILER_URL` around a run instead would take real notifications down for good on an interrupted one. Pinned by `test/mail.test.js`. |
...
| `armies.js` | `normalizeArmies(input)`, `armiesForUser(db, userId)` | The registered-army list behind `PUT /auth/me/armies`. `normalizeArmies` validates/cleans the whole-list body (throws plain Errors with a `.code`, caught into a 400 by the route); `armiesForUser` is the one ordered camelCase read, shared by `auth.js` and `stats.js`. Quick-pick data only — `game_players.faction_id` stays authoritative per match. |
