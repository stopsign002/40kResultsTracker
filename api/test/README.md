...
- **`test/*.test.js` — 156 unit cases.** Pure functions, no DB, no network, no
  filesystem. Runs in under two seconds.
- **`test/integration/*.test.js` — 122 cases.** Real HTTP against the running
  API and the **live** Postgres. Every row it creates belongs to a `zz_test_*`
  user, and it cleans up after itself.
...
scripts/test-live.sh                  # all 122 integration cases
...
Real HTTP, real sessions, real Postgres, real photo files on disk. 122 cases.
...
- **Name reference-data fixtures `ZZ …`.** `resolveGameLookups` auto-inserts a
  ...
  `primary_missions`, `deployment_maps`, `mission_rules` and `challenger_cards`,
...
1. `assertNoResidue()` — zero surviving test users, audit rows, banners,
   `deleted_items`, and zero `ZZ %` rows in any reference table.
...
3. No mission pack has grown a card, mission, layout or rule named like a
   fixture.
