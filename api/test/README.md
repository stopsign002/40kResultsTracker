...
- **`test/*.test.js` — 175 unit cases.** Pure functions, no DB, no network, no
  filesystem. Runs in under two seconds.
- **`test/integration/*.test.js` — 132 cases.** Real HTTP against the running
...
scripts/test-unit.sh                  # the 175 unit cases, --network none
scripts/test-live.sh                  # all 132 integration cases
