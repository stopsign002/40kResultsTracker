...
Eleven shared modules + a `views/` directory of per-route render functions. ES modules, no build step. Each script is loaded directly from a `<script type="module">` tag in `index.html`.
...
| `api.js` | `api`, `auth`, `reference`, `games`, `gameImages`, `drafts`, `draftImages`, `stats`, `admin`, `seasons`, `ratings` | Typed wrapper around `fetch`. All requests `credentials: 'same-origin'`. Throws Errors with `.status`, `.code` (server error code), and `.data` (full response body). Network failures throw with `status: 0`, `code: 'network'`. |
| `components.js` | `el`, `clear`, `toast`, `pill`, `fmtDate`, `fmtDuration`, `fmtScore`, `selectOptions`, `confirmModal`, `choiceModal`, `promptModal`, `buildModal` | DOM helpers. `choiceModal` is `confirmModal` with more than one way to say yes — it resolves to the chosen value or `null`, for cases like the between-rounds photo nudge where camera-vs-library has to be picked before the file picker opens. **Use these — don't template-string HTML.** `el(tag, attrs?, children?)` is the workhorse; `attrs.class`, `attrs.style` (object), `attrs.onClick`. |
...
| `game-rules.js` | `ROUNDS`, `DEFAULT_EDITION`, `MATCHED_PLAY_LAYOUTS`, `E11_PRIMARY_CAP`, `E11_SECONDARY_CAP`, `FORCE_DISPOSITIONS`, `PRIMARY_MATRIX`, `parseDuration`, `sumPrimary`, `sumSecondaries`, `sumSecondaryPoints`, `sumSecondaryForRound`, `secondaryRoundHeadroom`, `cumulativeTimeThrough`, `roundTimeFromClock`, `capLabel`, `calcTotal`, plus the per-battle-round ceilings `E11_PRIMARY_ROUND_CAP` / `E11_SECONDARY_ROUND_CAP` and the Fixed-secondary set `SECONDARY_MODES`, `FIXED_SECONDARY_COUNT`, `E11_FIXED_CARD_CAP`, `secondaryMode`, `isFixedMode`, `foldCardName`, `fixedCardTotal`, `fixedCardHeadroom` | 40k rules constants + score maths shared by `views/game-form.js` and `views/live-game.js` — which is also why the per-round cap logic lives here rather than in either view: both surfaces enforce it and must agree. **`calcTotal()` must track `computeFinalScores()` in `api/lib/game-scoring.js` exactly** — it is a hand-maintained mirror driving the live readout only; the server value is authoritative. |
| `mission-cards.js` | `loadMissionCards`, `missionLink`, `openMissionCard`, `openMissionBrowser`, `findPrimary`, `findSecondary`, `renderCardBody`, `fixedSecondaryOptions`, `isFixedOption` | The 11e mission deck as readable rules text, so a game can be played off the app with no physical cards. Lazy-loads `app/data/mission-cards-11e.json` (~51KB, committed; built by `scripts/build-mission-cards.py`) on first tap and caches it in module scope. Cards are matched **by name, case- and punctuation-insensitively** — GDC title-cases every word where `seed.sql` follows GW's casing. Prose is rendered as real text nodes and `<strong>` elements, never `innerHTML`. Also the authority on **which secondaries may be taken Fixed** (`fixedSecondaryOptions`), read off the deck's own `fixed` flag rather than hard-coded; `renderCardBody(card, kind, { mode })` shows only the relevant half of a dual-mode card. |
...
Photos, including the terrain shot, are **downscaled in the browser** before
...
- Reads never touch the API: `gameImages.url()` / `draftImages.url()` build plain
