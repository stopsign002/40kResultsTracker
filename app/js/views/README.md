...
| `game-detail.js` | `/games/:id` | Single game view with per-player breakdown. Secondaries are re-sorted **by round drawn** so the game reads back chronologically (entry is alphabetical). Hides the rounds grid entirely for a final-score-only game — an all-zero grid reads as "they scored nothing" rather than "nobody wrote it down". Army list in a collapsible `<details>` (`.detail-summary`), printed as stored — decoding happened at write time, so nothing is imported from `army-list.js` here. Photos panel (upload, incl. `.zip`; Cover / Map flags; lightbox) and a Terrain Layout panel; a photo carrying a caption gets a `.photo-badge.is-round` chip bottom-right. Admin-only Hide / Delete buttons (Delete uses `confirmModal`). |
