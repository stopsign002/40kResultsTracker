...
- **Confidence floor (the ranking key)** — the board ranks and headlines by `displayFloor` = `displayRating(rating − RANK_FLOOR_K·rd)` (K=1.5), *not* the raw mean. So a 1-game player who beats the best sits **low** (high RD → low floor) and climbs as they prove it, instead of leaping to #2. The raw mean shows as "est", "±" is the confidence. Applies in both models.
